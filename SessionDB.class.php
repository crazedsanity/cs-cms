...
class SessionDB extends Session {
	...
	public function __construct($automaticUpgrades=true, Database $db=null) {
		...
		if($automaticUpgrades === true) {
			$x = new Upgrade(dirname(__FILE__) .'/VERSION', dirname(__FILE__) .'/upgrades/upgrade.ini', $this->db);
			$x->check_versions();
		}
		
		//create a logger (this will automatically cause any upgrades to happen).
		$this->logger = new Logger($this->db, 'Session DB', false);
	...
	public function sessdb_write($sid, $data) {
		if(is_string($sid) && strlen($sid) >= 20) {
			...
			try {
				...
				#$this->do_log(__METHOD__ .": finished (". $type ."), DATA::: ". cs_global::debug_print($data,0), 'debug');
			}
			catch(exception $e) {
	...
	protected function do_log($message, $type, $uid=null) {
		try {
			...
			if(!is_object($this->logger)) {
				...
				$this->logger = new Logger($newDB, $this->logCategory);
			}
	...
	protected function exception_handler($message, $throwException=false) {
		...
		try {
			...
			if(function_exists('cs_debug_backtrace')) {
				$message .= "\n\nBACKTRACE:::\n". \crazedsanity\debug_backtrace(0);
			}
	...
	protected function flogger($msg) {
		...
		try {
			...
			$msg = '['. SessionDB::$runDate .'] '. \crazedsanity\get_where_called() . " -- ". $msg;
