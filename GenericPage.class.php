...
class GenericPage extends baseAbstract {
	...
	public function mini_parser($template, $repArr, $b='%%', $e='%%') {
		return(cs_global::mini_parser($template, $repArr, $b, $e));
	}//end mini_parser()
	//---------------------------------------------------------------------------------------------
	...
	public function print_page($stripUndefVars=1) {
		...
		if(!strlen($out)) {
			cs_global::debug_print($out);
			cs_global::debug_print($this->mainTemplate);
			cs_global::debug_print("MANUAL FILE CONTENTS::: ". htmlentities(file_get_contents($this->mainTemplate)));
		...
		while(preg_match_all('/\{.\S+?\}/', $out, $tags) && $numLoops < 10) {
			$out = cs_global::mini_parser($out, $this->templateVars, '{', '}');
			$numLoops++;
		}
